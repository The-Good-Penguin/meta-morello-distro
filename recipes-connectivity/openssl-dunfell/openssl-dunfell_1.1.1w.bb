...
MULTILIB_SCRIPTS = "openssl-bin:${bindir}/c_rehash"
...
do_install () {
	oe_runmake DESTDIR="${D}" MANDIR="${mandir}" MANSUFFIX=ssl-1.1 install
  ...
  mv ${D}${libdir}/pkgconfig/libcrypto.pc ${D}${libdir}/pkgconfig/libcrypto.1.1.pc
  mv ${D}${libdir}/pkgconfig/libssl.pc ${D}${libdir}/pkgconfig/libssl.1.1.pc

	ln -rs ${TARGET_INSTALL_DIR}${libdir}/libcrypto.so.1.1 ${D}${libdir}/libcrypto.1.1.so
	ln -rs ${TARGET_INSTALL_DIR}${libdir}/libssl.so.1.1 ${D}${libdir}/libssl.1.1.so

  install -d ${D}${includedir}/openssl.1.1
  mv ${D}${includedir}/openssl ${D}${includedir}/openssl.1.1

  rm -f ${D}${libdir}/libcrypto.so
  rm -f ${D}${libdir}/libcrypto.a
  rm -f ${D}${libdir}/libssl.so
  rm -f ${D}${libdir}/libssl.a
}
...
PACKAGES =+ "libcrypto-dunfell libssl-dunfell ${PN}-engines"

FILES:libcrypto-dunfell = "${libdir}/libcrypto.so.1.1 ${libdir}/libcrypto.1.1.so"
FILES:libssl-dunfell = "${libdir}/libssl.so.1.1 ${libdir}/libssl.1.1.so"
...
FILES:${PN}-includedir += "${includedir}/openssl.1.1"
...
PROVIDES += "openssl-dunfell-includedir"
